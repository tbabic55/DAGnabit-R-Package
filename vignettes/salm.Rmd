...
# Pick whichever parser is available
if (exists("parse_bugs_model", mode = "function")) {
  parser_fn <- parse_bugs_model
} else if (exists("parse_bugs_to_nodes_edges", mode = "function")) {
  parser_fn <- parse_bugs_to_nodes_edges
} else {
  parser_fn <- NULL
}
...
# prefer packaged copies, fall back to inst/ paths during dev
nodes_path <- system.file("extdata/examples/salm/nodes.csv", package = pkg)
edges_path <- system.file("extdata/examples/salm/edges.csv", package = pkg)
if (!nzchar(nodes_path)) nodes_path <- "inst/extdata/examples/salm/nodes.csv"
if (!nzchar(edges_path)) edges_path <- "inst/extdata/examples/salm/edges.csv"
...
if (!file.exists(nodes_path) || !file.exists(edges_path)) {
  ...
  knitr::kable(
    data.frame(
      ...
      nodes    = c(nrow(salm_nodes),  nrow(salm_nodes_fixture)),
      edges    = c(nrow(salm_edges),  nrow(salm_edges_fixture))
    ),
    caption = "Node/edge counts: parser vs Salm fixture."
  )
}
```
